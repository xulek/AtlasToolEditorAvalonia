...
namespace AtlasToolEditorAvalonia.ModulusCryptor
{
    public class MARSCipher : ICipher
    {
        ...
        private static readonly uint[] s_box = new uint[512]
        {
  ...
  0xab561187, 0x14eea0f0, 0xdf0d4164, 0x19af70ee
        ...
        private uint[] l_key = new uint[40];
        ...
        {
            if (len % BLOCK_SIZE != 0)
                throw new ArgumentException("Data length must be a multiple of block size.");
        ...
        public void SetKey(byte[] in_key)
        {
            uint[] T = new uint[15];
            int key_len = in_key.Length * 8;

            // Copiar la clave en el array T
            for (int i = 0; i < in_key.Length / 4; i++)
            {
                T[i] = ToUInt32LittleEndian(in_key, i * 4);
            }

            T[in_key.Length / 4] = (uint)(in_key.Length / 4);

            // Expandir la clave en varias rondas como en C++
            for (int j = 0; j < 4; j++)
            {
                // Transformación lineal
                for (int i = 0; i < 15; i++)
                {
                    T[i] ^= Rotl(T[(i + 8) % 15] ^ T[(i + 13) % 15], 3) ^ (uint)(4 * i + j);
                }

                // Cuatro rondas de mezcla
                for (int k = 0; k < 4; k++)
                {
                    for (int i = 0; i < 15; i++)
                    {
                        T[i] = Rotl(T[i] + s_box[T[(i + 14) % 15] % 512], 9);
                    }
                }

                // Almacenar las próximas 10 palabras de clave
                for (int i = 0; i < 10; i++)
                {
            ...
            // Modificar las palabras clave de multiplicación como en C++
            for (int i = 5; i < 37; i += 2)
            {
                ...
                if (m != 0)
                {
                    w ^= (Rotl(s_box[265 + (l_key[i] & 3)], (int)(l_key[i - 1] & 31)) & m);
                }
        ...
        private uint GenMask(uint x)
        {
            uint m = (~x ^ (x >> 1)) & 0x7fffffff;
        ...
        private void DecryptBlock(byte[] in_blk, byte[] out_blk)
        {
            uint d = ToUInt32LittleEndian(in_blk, 0) + l_key[36];
            uint c = ToUInt32LittleEndian(in_blk, 4) + l_key[37];
            uint b = ToUInt32LittleEndian(in_blk, 8) + l_key[38];
            uint a = ToUInt32LittleEndian(in_blk, 12) + l_key[39];

            // Aplicar las mezclas
            F_Mix(ref a, ref b, ref c, ref d); a += d;
            F_Mix(ref b, ref c, ref d, ref a); b += c;
            F_Mix(ref c, ref d, ref a, ref b);
            F_Mix(ref d, ref a, ref b, ref c);
            F_Mix(ref a, ref b, ref c, ref d); a += d;
            F_Mix(ref b, ref c, ref d, ref a); b += c;
            F_Mix(ref c, ref d, ref a, ref b);
            F_Mix(ref d, ref a, ref b, ref c);

            // Aplicar las rondas inversas
            R_Ktr(ref a, ref b, ref c, ref d, 34); R_Ktr(ref b, ref c, ref d, ref a, 32);
            R_Ktr(ref c, ref d, ref a, ref b, 30); R_Ktr(ref d, ref a, ref b, ref c, 28);
            R_Ktr(ref a, ref b, ref c, ref d, 26); R_Ktr(ref b, ref c, ref d, ref a, 24);
            R_Ktr(ref c, ref d, ref a, ref b, 22); R_Ktr(ref d, ref a, ref b, ref c, 20);
            R_Ktr(ref a, ref d, ref c, ref b, 18); R_Ktr(ref b, ref a, ref d, ref c, 16);
            R_Ktr(ref c, ref b, ref a, ref d, 14); R_Ktr(ref d, ref c, ref b, ref a, 12);
            R_Ktr(ref a, ref d, ref c, ref b, 10); R_Ktr(ref b, ref a, ref d, ref c, 8);
            R_Ktr(ref c, ref b, ref a, ref d, 6); R_Ktr(ref d, ref c, ref b, ref a, 4);

            // Aplicar las mezclas inversas
            B_Mix(ref a, ref b, ref c, ref d);
            B_Mix(ref b, ref c, ref d, ref a); c -= b;
            B_Mix(ref c, ref d, ref a, ref b); d -= a;
            B_Mix(ref d, ref a, ref b, ref c);
            B_Mix(ref a, ref b, ref c, ref d);
            B_Mix(ref b, ref c, ref d, ref a); c -= b;
            B_Mix(ref c, ref d, ref a, ref b); d -= a;
            B_Mix(ref d, ref a, ref b, ref c);

            uint[] outWords = new uint[4];
            outWords[0] = d - l_key[0];
            outWords[1] = c - l_key[1];
            outWords[2] = b - l_key[2];
            outWords[3] = a - l_key[3];

            WriteUInt32LittleEndian(outWords[0], out_blk, 0);
            WriteUInt32LittleEndian(outWords[1], out_blk, 4);
            WriteUInt32LittleEndian(outWords[2], out_blk, 8);
            WriteUInt32LittleEndian(outWords[3], out_blk, 12);
        }
        ...
        public static void WriteUInt32LittleEndian(uint value, byte[] buffer, int offset)
        {
        ...
        private void F_Mix(ref uint a, ref uint b, ref uint c, ref uint d)
        {
            uint r = Rotr(a, 8);
            b ^= s_box[a & 255];
            b += s_box[(r & 255) + 256];
        ...
        private void B_Mix(ref uint a, ref uint b, ref uint c, ref uint d)
        {
            uint r = Rotl(a, 8);
            b ^= s_box[(a & 255) + 256];
            c -= s_box[r & 255];
        ...
        private void F_Ktr(ref uint a, ref uint b, ref uint c, ref uint d, int i)
        {
            uint m = a + l_key[i];
            a = Rotl(a, 13);
            uint r = a * l_key[i + 1];
        ...
        private void R_Ktr(ref uint a, ref uint b, ref uint c, ref uint d, int i)
        {
            uint r = a * l_key[i + 1];
            a = Rotr(a, 13);
            uint m = a + l_key[i];
